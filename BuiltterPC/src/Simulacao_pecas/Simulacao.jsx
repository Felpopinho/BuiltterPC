import { Box, Button, Divider, Typography, LinearProgress, Modal, IconButton, Stepper, Step, StepButton } from "@mui/material"
import { Molde } from "./Molde"
import { previewUser, templateImagens, iconSection } from "../script"
import { Fragment, useState } from "react"
import { Close } from "@mui/icons-material";
import { ProdutosMolde } from "./CriarMolde";
import { MoldeResultUm } from "./MoldeResult";
...
export function Simulacao(props){
    ...
    const [moldeAtual, setMoldeAtual] = useState()

    const [moldeOpen, setMoldeOpen] = useState(false);
    const handleOpenModal = () =>{
        props.logado === false ? props.setOpenAviso(true) : setMoldeOpen(true);
    }
    ...
    return <div className="simulacao_container" id="Simulacao">
        <Box className="inicioSim">
            ...
            <Button className="infoSim" variant="contained" sx={{height: '5vh'}} onClick={handleOpenModal}>
                Criar molde
            </Button>
        </Box>
        ...
        <Box className="moldeBoxContainer">
            ...
            {props.simulacoes === "" ?
                ...
                <div className="divs" style={{display: 'flex', width: "100%"}}>
                    {props.logado === false ? 
                        (<Fragment><Typography variant="h3" fontWeight={700} color={"white"} display={"flex"} justifyContent={"center"} width={"100%"}>Crie uma conta ou faça login para acessar os moldes</Typography></Fragment>) : 
                        (<Fragment>{Array.from(props.simulacoes).length !== 0 ?
                            Array.from(props.simulacoes).map(molde => molde.userId === previewUser.idUser ? (<Molde getData={props.getData} produtos={props.produtos} key={molde.id} nome={molde.nome} molde={molde} 
                            id={molde.id} mae={molde.mae} pro={molde.pro} mem={molde.mem} arm={molde.arm} vid={molde.vid} fon={molde.fon} handleOpenAlert={props.handleOpenAlert}
                            idMae={idMae} idPro={idPro} idMem={idMem} idArm={idArm} idFon={idFon} idVid={idVid} setIdNome={setIdNome} idNome={idNome} section={section} setSection={setSection}
                            setIdMae={setIdMae} setIdPro={setIdPro} setIdMem={setIdMem} setIdArm={setIdArm} setIdVid={setIdVid} setIdFon={setIdFon}
                            editMolde={editMolde} setEditMolde={setEditMolde} setCompleted={setCompleted} setPassar={setPassar} setMoldeOpen={setMoldeOpen}
                            handleOpenResult={handleOpenResult} setResultOpen={setResultOpen} resultOpen={resultOpen} setMoldeAtual={setMoldeAtual} deleteMolde={deleteMolde}/>) : "") : 
                            <Typography variant="h3">Você ainda não criou nenhum molde</Typography>}
                        </Fragment>)
                    }
                </div>
            }
            ...
            <Modal open={moldeOpen}>
                <Box sx={{position: 'absolute',top: '50%',left: '50%',transform: 'translate(-50%, -50%)',bgcolor: '#f7fbff',boxShadow: 24}} className="moldeModal">
                    ...
                    <Fragment>
                        ...
                        editMolde={editMolde} edit={editMolde} setCriando={setCriandoMolde} criando={criandoMolde} simulacao_id={props.id} setResultOpen={setResultOpen}/>
                    </Fragment> :
                    <Fragment>
